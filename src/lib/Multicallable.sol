...
abstract contract Multicallable is IMulticallable {
    ...
    ///
    /// Normally it should not be payable in order to prevent double-spend attacks, but
    /// here we know that we do not use msg.value in the internal accounting so it is safe.
    ///
